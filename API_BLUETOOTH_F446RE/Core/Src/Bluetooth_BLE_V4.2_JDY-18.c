 ...
 *      Resumo: Esta API foi desenvolvida como trabalho da disciplina de Programação de Sistemas Embarcados da UFMG
 ...
 *      @brief This is the source file for the API of the Bluetooth_BLE_V4.2_JDY-18 module.
 *      	   Datasheet: https://manuals.plus/bluetooth-module/bluetooth-jdy-18-4-2-ble-module-usage-manual#axzz7jKHPWZQl
 */
...
uint16_t ble_send_info(I2C_HandleTypeDef *hi2c, const uint8_t *ble_dado, uint8_t dsize) {
	if (ble_write(hi2c, ble_writeapp_adress, (uint8_t *) ble_dado, dsize ) == HAL_OK) {
		ble_reset(hi2c);
		return 1;
	} else
		return 0;
